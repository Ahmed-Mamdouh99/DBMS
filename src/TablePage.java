...
class TablePage implements Serializable {
  ...
  // static methods
  static TablePage loadPage(File pageFile) throws DBAppException { // Completed
    try {
  ...
  static TablePage createPage(File pageFile) throws DBAppException { // Completed
  ...
  // Instance methods
  Hashtable<String, Comparable> peek() { // Completed
    return records.get(0);
  }

  private void writeToDisk() throws DBAppException { // Completed
  ...
  Hashtable<String, Comparable> insert(Hashtable<String, Comparable> newRecord, String key) throws DBAppException { // Completed
    int index = 0;
    ...
    for (; index < records.size(); index++) {
      ...
      if (keyValue.compareTo(currentValue) >= 0) {
        if (records.size() == maxSize) {
          ...
          Hashtable<String, Comparable> tmp = records.remove(index);
  ...
  int delete(Hashtable<String, Comparable> mask) throws DBAppException { // Method completed
    int deleted = 0;
  ...
  Hashtable<String, Comparable> get(int index) { // Completed
